#!/usr/bin/env python3
"""Dump disassembly around 0x08901310 (HUD code cave hook point) and
the sprite render function 0x08901168 to understand stack layout."""
...
# Dump full sprite render function 0x08901168 through 0x08901400
print("=== 0x08901168 FULL SPRITE RENDER FUNCTION ===")
print("(includes 0x08901310 hook point)")
for addr in range(0x08901168, 0x08901400, 4):
    ...
    elif addr == 0x08901314:
        marker = " <<<< HUD CAVE HOOK (NOP'd)"
    elif addr == 0x08901168:
        marker = " <<<< FUNCTION ENTRY"
    # Track stack operations
    if "$sp" in d and ("sw " in d or "lw " in d or "addiu" in d):
        marker += " [STACK]"
    if "$ra" in d:
        marker += " [RA]"
...
# Also dump the callers in the overlay that render HP bars
# From dump_hp_bar_area.py: HP_BAR_SUB is at 0x09D61838
print("\n\n=== 0x09D61830 area (HP bar caller context) ===")
for addr in range(0x09D61800, 0x09D61900, 4):
    off = psp_to_offset(addr)
    if off + 4 > len(data): break
    instr = read_u32(data, off)
    d = disasm(instr, addr)
    marker = ""
    if addr == 0x09D61830:
        marker = " <<<< RETURN ADDRESS CHECKED BY CAVE"
    if "jal " in d and "jalr" not in d:
        target = (instr & 0x03FFFFFF) << 2
        marker += f" [CALL -> 0x{target:08X}]"
    print(f"  0x{addr:08X}: 0x{instr:08X}  {d}{marker}")

# And the sharpness/clock caller at 0x09D6C728
print("\n\n=== 0x09D6C720 area (sharpness/clock caller context) ===")
for addr in range(0x09D6C700, 0x09D6C800, 4):
    off = psp_to_offset(addr)
    if off + 4 > len(data): break
    instr = read_u32(data, off)
    d = disasm(instr, addr)
...
# Also dump what's around 0x08842ED4 (minimap 50% eboot patch area)
print("\n\n=== 0x08842EC0 - 0x08844340 (minimap eboot patch area) ===")
print("Checking if these are CODE or DATA...")
for addr in range(0x08842EC0, 0x08842F20, 4):
    off = psp_to_offset(addr)
    if off + 4 > len(data): break
    instr = read_u32(data, off)
    d = disasm(instr, addr)
    marker = ""
    if addr in [0x08842ED4, 0x08842ED8]:
        marker = " <<<< MINIMAP SCALE PATCH"
    print(f"  0x{addr:08X}: 0x{instr:08X}  {d}{marker}")
print("  ...")
for addr in range(0x08844300, 0x08844320, 4):
    off = psp_to_offset(addr)
    if off + 4 > len(data): break
    instr = read_u32(data, off)
    d = disasm(instr, addr)
    marker = ""
    if addr in [0x08844300, 0x08844314, 0x08844318]:
        marker = " <<<< MINIMAP ICON PATCH"
    print(f"  0x{addr:08X}: 0x{instr:08X}  {d}{marker}")
