#!/usr/bin/env python3
"""Dump sub-functions called from 0x09D68040 to find HP bar render calls."""
...
def dump_func(start, max_len=0x800, label=""):
    print(f"\n=== {label} 0x{start:08X} ===")
    for addr in range(start, start + max_len, 4):
        ...
        if "jal " in d and "jalr" not in d:
            ...
            if target in RENDER_TARGETS:
                marker = f" *** {RENDER_TARGETS[target]} ***"
            elif 0x09C50000 <= target <= 0x09DA0000:
                marker = " [OVL]"
        if "jalr" in d:
            marker = " [INDIRECT]"
        if "$t0" in d and ("addiu" in d or "addu" in d):
            marker += " [T0_SET]"
        if "$t1" in d and ("addiu" in d or "addu" in d):
            marker += " [T1_SET]"
        show = marker or "jr $ra" in d or "jalr" in d or ("jal " in d and "jalr" not in d)
        if show:
            print(f"  0x{addr:08X} (CW 0x{cw:07X}): {d}{marker}")
        if "jr $ra" in d:
            break

# Sub-functions called by 0x09D68040
for func, name in [
    (0x09D62D20, "0x09D62D20 (called from 8040)"),
    (0x09D62828, "0x09D62828 (called from 8040)"),
    (0x09D63668, "0x09D63668 (called from 8040)"),
    (0x09D5F778, "0x09D5F778 (called from 8040)"),
    (0x09D67D80, "0x09D67D80 (called from 8040)"),
    (0x09D5EDF8, "0x09D5EDF8 (called from B53C)"),
    (0x09D620AC, "0x09D620AC (called from B3F0)"),
    (0x09D6A858, "0x09D6A858 (called from B3F0)"),
    (0x09D5FF70, "0x09D5FF70 (called from 43B0)"),
    (0x09D5FFEC, "0x09D5FFEC (called from 7AEC)"),
]:
    dump_func(func, 0x800, name)
