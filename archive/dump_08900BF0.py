#!/usr/bin/env python3
"""Dump 0x08900BF0 to see how it reads X/Y from entries and if $t0-$t3 are offsets."""
...
print("=== 0x08900BF0 (background sprite renderer?) ===")
print("Called with: $a0=ctx, $a1=entry_A, $a2=entry_B, $a3=?, $t0-$t3=0")
for addr in range(0x08900BF0, 0x08901000, 4):
    off = psp_to_offset(addr)
    if off + 4 > len(data): break
    instr = read_u32(data, off)
    d = disasm(instr, addr)
    marker = ""
    if "($a1)" in d: marker += " <<< ENTRY_A"
    if "($a2)" in d: marker += " <<< ENTRY_B"
