#!/usr/bin/env python3
"""Dump the FULL LOOP2 and LOOP3 including code after first jr $ra."""
...
def dump_range(start, end, title):
    ...
    for addr in range(start, end, 4):
        ...
        # Detect function prologue (addiu $sp, $sp, -N)
        op = instr >> 26; rs = (instr >> 21) & 0x1F; rt = (instr >> 16) & 0x1F
        if op == 0x09 and rs == 29 and rt == 29 and (instr & 0xFFFF) >= 0x8000:
            if addr != start:
                marker += " *** FUNC START ***"
        print(f"  0x{addr:08X}: {d}{marker}")
        if "jr $ra" in d:
            jr_count += 1
            if jr_count >= 3:  # Stop after 3 jr $ra (multiple code paths)
                break

# LOOP2 full - from 0x09D62B30 to well past 0x09D62BD4
dump_range(0x09D62B30, 0x09D62F00, "LOOP2 (0x09D62B30) FULL")

# LOOP3 full - from 0x09D6309C to well past 0x09D63120
dump_range(0x09D6309C, 0x09D63800, "LOOP3 (0x09D6309C) FULL")

# Now let's look at the PARENT function more carefully around the loops
# Focus on the player list path: what happens between 0x09D6C728 and the loops
print("\n\n=== PARENT PLAYER LIST PATH (0x09D6C718-0x09D6C9A0) ===")
for addr in range(0x09D6C718, 0x09D6C9A0, 4):
    off = psp_to_offset(addr)
    if off + 4 > len(data): break
    instr = read_u32(data, off)
    d = disasm(instr, addr)
...
# Also check what 0x09D62880 does - it was found as a render-calling function
# and it's in the right address range to be related to the player list
dump_range(0x09D62880, 0x09D62B30, "0x09D62880 (UNKNOWN RENDER CALLER)")

# Check 0x09D62110 too
dump_range(0x09D62110, 0x09D62280, "0x09D62110 (UNKNOWN RENDER CALLER)")

# Check 0x09D60660
dump_range(0x09D60660, 0x09D60900, "0x09D60660 (UNKNOWN RENDER CALLER)")

# Check 0x09D609A4
dump_range(0x09D609A4, 0x09D60C00, "0x09D609A4 (UNKNOWN RENDER CALLER)")

# Check 0x09D60C74
dump_range(0x09D60C74, 0x09D60D88, "0x09D60C74 (UNKNOWN RENDER CALLER)")

# Check 0x09D610B8
dump_range(0x09D610B8, 0x09D61248, "0x09D610B8 (UNKNOWN RENDER CALLER)")
