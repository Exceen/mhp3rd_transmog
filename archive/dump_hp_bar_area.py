#!/usr/bin/env python3
"""Dump the area 0x09D622xx-0x09D625xx to understand the HP bar rendering chain.
Also find the function containing 0x09D62418."""
...
KNOWN = {
    0x09D61838: "HP_BAR_SUB(3xsprite1000+bg)",
    0x09D61558: "HP_BAR_SUB2(2xsprite1000+2xsprite1168)",
    0x09D614EC: "BG_SUB(bg_BF0)",
    0x09D62110: "SPRITE_F8F0_SUB",
...
# Dump from 0x09D622D4 to 0x09D62600
print("=== 0x09D62200 - 0x09D62600 FULL DUMP ===")
for addr in range(0x09D62200, 0x09D62600, 4):
    ...
    if "$s7" in d: marker += " [s7_X]"
    if "$fp" in d: marker += " [fp_Y]"
    if "$t0" in d: marker += " [t0]"
    if "$t1" in d: marker += " [t1]"
    # Highlight function prologues
    op = instr >> 26; rs = (instr >> 21) & 0x1F; rt = (instr >> 16) & 0x1F
    if op == 0x09 and rs == 29 and rt == 29 and (instr & 0xFFFF) >= 0x8000:
        marker += " *** FUNC START ***"
    print(f"  0x{addr:08X} (CW 0x{cw:07X}): {d}{marker}")

# Also dump 0x09D624D4 which we know is a function
print("\n\n=== 0x09D624D4 FULL DUMP (the function that supposedly has $s7/$fp X/Y) ===")
for addr in range(0x09D624D4, 0x09D62B30, 4):
    off = psp_to_offset(addr)
    if off + 4 > len(data): break
    instr = read_u32(data, off)
    d = disasm(instr, addr)
