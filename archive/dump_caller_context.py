#!/usr/bin/env python3
"""Dump the parent function 0x09D6C498 around the item selector call at 0x09D6C988.
Focus on branches that could gate the call."""
...
# Dump a wide range around the call site at 0x09D6C988
# The parent function starts at 0x09D6C498
# Focus on the region leading up to the call and any branches that jump over it
print("=== Parent function around item selector call (0x09D6C900-0x09D6CA00) ===")
print("  (looking for branches that skip the jal 0x09D60D88 at 0x09D6C988)")
print()

# First, find all branches in the function that could jump OVER 0x09D6C988
print("=== Branches in 0x09D6C498-0x09D6CA00 that target >= 0x09D6C98C ===")
for addr in range(0x09D6C498, 0x09D6CA00, 4):
    ...
    if op in [0x04, 0x05, 0x06, 0x07]:  # beq, bne, blez, bgtz
        ...
        # Branches that skip over the call
        if addr < 0x09D6C988 and branch_target > 0x09D6C988:
            d = disasm(instr, addr)
            print(f"  0x{addr:08X}: {d}  (skips over item sel call)")
...
# Also find jumps (j instruction) that skip over the call
print("\n=== Jumps that skip over item sel call ===")
for addr in range(0x09D6C498, 0x09D6CA00, 4):
    off = psp_to_offset(addr)
    instr = read_u32(data, off)
    op = instr >> 26
    if op == 0x02:  # j
        target = (instr & 0x03FFFFFF) << 2
        if addr < 0x09D6C988 and target > 0x09D6C988:
            d = disasm(instr, addr)
            print(f"  0x{addr:08X}: {d}  (jumps past item sel call)")

# Now dump detailed context around the call (0x09D6C930-0x09D6C9A0)
print("\n=== Detailed dump 0x09D6C930-0x09D6C9A0 ===")
for addr in range(0x09D6C930, 0x09D6C9A0, 4):
    off = psp_to_offset(addr)
    instr = read_u32(data, off)
    d = disasm(instr, addr)
    marker = " <<<" if addr == 0x09D6C988 else ""
    print(f"  0x{addr:08X}: 0x{instr:08X}  {d}{marker}")

# Now dump around 0x09D6C800-0x09D6C830 to understand the jump target at 0x09D6C80C
print("\n=== Code at 0x09D6C7F0-0x09D6C830 (jump target area) ===")
for addr in range(0x09D6C7F0, 0x09D6C830, 4):
    off = psp_to_offset(addr)
    instr = read_u32(data, off)
    d = disasm(instr, addr)
