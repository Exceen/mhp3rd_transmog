#!/usr/bin/env python3
"""Dump the rendering orchestrator function that calls all per-player loops."""
...
KNOWN = {
    ...
    0x09D6380C: "PER_PLAYER_LOOP1(names/icons)",
...
# Find function start before 0x09D6C720
func_start = 0x09D6C660  # fallback
for a in range(0x09D6C720, 0x09D6C498, -4):
    off = psp_to_offset(a)
    instr = read_u32(data, off)
    op = instr >> 26; rs = (instr >> 21) & 0x1F; rt = (instr >> 16) & 0x1F
    if op == 0x09 and rs == 29 and rt == 29 and (instr & 0xFFFF) >= 0x8000:
        print(f"Function start found at 0x{a:08X}")
        func_start = a
        break

# Dump from function start to end
print(f"\n=== RENDER ORCHESTRATOR 0x{func_start:08X} ===")
returns = 0
for addr in range(func_start, func_start + 0x600, 4):
    ...
    if "$s4" in d or "$s5" in d: marker += " [s4/s5]"
