#!/usr/bin/env python3
"""Disassemble 0x08900BF0 to check if it calls 0x0890000C internally."""
...
# Disassemble 0x08900BF0 — the batch sprite renderer
print("=" * 70)
print("=== Function 0x08900BF0 (batch sprite renderer) ===")
print("=" * 70)

known = {
    0x0890000C: "SPRITE_SINGLE",
    0x088FFF0C: "HP_BAR_RENDER",
    0x08900BF0: "SELF (recursive?)",
    0x088FAC54: "VISIBILITY_CHECK",
    0x088FF8F0: "SPRITE_DRAW_CORE",
    0x088FF1F0: "SPRITE_BASE_RENDER",
    0x08901168: "SPRITE_RENDER_OFFSET",
}

for addr in range(0x08900BF0, 0x08900BF0 + 0x200, 4):
...
# Also check what functions are between 0x0890000C and 0x08900BF0
# to understand the call chain
print(f"\n{'='*70}")
print("=== Functions between 0x089000C0 and 0x08900BF0 ===")
print(f"{'='*70}")
# Find function prologues (addiu $sp, $sp, -XX)
for addr in range(0x089000C0, 0x08900BF0, 4):
    off = psp_to_offset(addr)
    instr = read_u32(data, off)
    op = instr >> 26
    rs = (instr >> 21) & 0x1F
